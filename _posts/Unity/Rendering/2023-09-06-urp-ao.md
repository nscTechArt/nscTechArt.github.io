...
我们在观察空间内完成SSAO的计算，所以，我们需要根据当前片段的屏幕空间坐标与深度值，来还原出观察空间下由相机指向当前片段的向量。具体的还原过程可以回顾[这篇博客](https://lovewithyou.tech/posts/urp-reconstruction-from-depth/)，这里就不再赘述了。这部分的代码如下：
